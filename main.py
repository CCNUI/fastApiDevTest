...
from models import Base, Teacher as TeacherModel, Student as StudentModel  # 导入数据库模型
...
def create_teacher(teacher: TeacherCreate, db: Session = Depends(get_db)):
    try:
        ...
        return Teacher.from_orm(db_teacher)  # 转换为 Pydantic 模型
...
def read_teachers(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    try:
        ...
        return [Teacher.from_orm(teacher) for teacher in teachers]  # 转换为 Pydantic 模型
    except Exception as e:
...
def read_teacher(teacher_id: int, db: Session = Depends(get_db)):
    ...
    return Teacher.from_orm(teacher)  # 转换为 Pydantic 模型
...
def create_student(student: StudentCreate, db: Session = Depends(get_db)):
    try:
        ...
        return Student.from_orm(db_student)  # 转换为 Pydantic 模型
    except Exception as e:
...
def read_students(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    ...
    return [Student.from_orm(student) for student in students]  # 转换为 Pydantic 模型
...
def read_student(student_id: int, db: Session = Depends(get_db)):
    ...
    return Student.from_orm(student)  # 转换为 Pydantic 模型
